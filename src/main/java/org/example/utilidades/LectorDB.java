...
public class LectorDB {
    ...
    public void levantarPronosticos() {
        try {
            ...
            Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/Entrega3?allowPublicKeyRetrieval=true&useSSL=false", "root", "Saimon0310");
	...
	public boolean acertoTodosLosPronosticosRonda(Persona p, int ronda ) {
		...
		List<Pronostico> pronosticos1 = this.pronosticos.stream().filter(pronostico ->pronostico.getPersona().equals(p)).toList();
		
		pronosticos1 = pronosticos.stream().filter(pronostico -> pronostico.getRonda().getNumeroRonda() == ronda).toList();
	...
	public boolean acertoTodosLosPronosticosFase(Persona p, int fase ) {
		...
		pronosticos1 = pronosticos.stream().filter(pronostico -> pronostico.getFase().getNumeroFase() == fase).toList();
